...
def get_target_dist(model, dataloader):
    ...
    for i, (inputs, target, group_index) in enumerate(dataloader):
        ...
        inputs = inputs.to(device)
        # inference
        dist = distance_to_decision_boundary(model, inputs)
        # one-hot to index
        #pred = torch.argmax(outputs, dim=1)
        # to list
        preds = list(dist.cpu())
...
@torch.no_grad()
def get_target_dist_in_group(model, dataloader):
    groups_pred = [[] for _ in range(num_group)]
    groups_true = [[] for _ in range(num_group)]
    if device == 'cuda':
        model = torch.nn.DataParallel(model).to(device)
...
def distance_to_decision_boundary(model, x):
    ...
    logits, fea = model(x)
    #print(logits)
    # Calculate loss with respect to the input x
...
def run(args, run_idx):
    ...
    ''' get target preds '''
    target_train_pred, target_train_true = get_target_dist_in_group(target_model, target_train_loader)
    target_test_pred, target_test_true = get_target_dist_in_group(target_model, target_test_loader)
    ''' get attack preds '''
    ...
    for i in range(num_group):
        ...
        target_train_df += [pd.DataFrame({'0': [float(d) for d in preds]})]
    preds, trues = get_target_dist(target_model, target_train_loader)
    #mg = MetricGenerator()
    #mg.update(preds, trues)
    target_train_df_all = pd.DataFrame({'0': [float(d) for d in preds]})
    ''' target test acc '''
    target_test_df = []
    for i in range(num_group):
        ...
        #mg = MetricGenerator()
        #mg.update(preds, trues)
        target_test_df += [pd.DataFrame({'0': [float(d) for d in preds]})]
    preds, trues = get_target_dist(target_model, target_test_loader)
    #mg = MetricGenerator()
    #mg.update(preds, trues)
    target_test_df_all = pd.DataFrame({'0': [float(d) for d in preds]})
    ''' attack acc in group '''
    metrics_list = []
    #for i in range(num_group):
    #    preds = groups_pred[i]
...
def main(args, num_run):
    ...
    for i in range(num_run):
        ...
        if not os.path.exists(save_path):
            os.mkdir(save_path)
        save_path = f'{save_path}/csv_dist_para'
        if not os.path.exists(save_path):
            os.mkdir(save_path)
    ...
    for i in range(num_group):
        save_path = f'{save_base_path}/{arch_conf}/{learn_method}_{attack_method}_k{args.k}/csv_dist_para/{loss_conf}/{arch_conf}_target_train_gruop_{i}.csv'
        target_train_df[i].to_csv(save_path)

    save_all_path = f'{save_base_path}/{arch_conf}/{learn_method}_{attack_method}_k{args.k}/csv_dist_para/{loss_conf}/{arch_conf}_target_train.csv'
    target_train_df_all.to_csv(save_all_path)

    for i in range(num_group):
        save_path = f'{save_base_path}/{arch_conf}/{learn_method}_{attack_method}_k{args.k}/csv_dist_para/{loss_conf}/{arch_conf}_target_test_gruop_{i}.csv'
    ...
    save_all_path = f'{save_base_path}/{arch_conf}/{learn_method}_{attack_method}_k{args.k}/csv_dist_para/{loss_conf}/{arch_conf}_target_test.csv'
