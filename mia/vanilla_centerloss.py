...
from representation.loss.centerloss import CECL
...
class VanillaMia_Centerloss(VanillaMia):
    def __init__(
            self,
        ...
        super(VanillaMia_Centerloss, self).__init__(
            target_model, shadow_models, attack_model,
            target_train, target_test, shadow_train_list, shadow_test_list,
    ...
    def train_target_model(self, args):
        ...
        ''' others '''
        criterion = CECL(num_classes[args.dataset], thelambda=self.loss_confs['lambda'], feat_dim=self.loss_confs['fea_dim']).to(args.device)
    ...
    def classification_train(self, args, model, dataloader, optimizer, criterion, current_epoch, logger):
        ...
        for i, (inputs, target) in enumerate(dataloader):
            ...
            loss = criterion(output, target, fea)
            total_loss = loss
            loss.item()
